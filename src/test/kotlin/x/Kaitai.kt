...
//            "/home/ununhexium/dev/kotlin/foiegras/analysis/A.class",
        "/home/ununhexium/dev/kotlin/foiegras/generated/base/j/newobjectasfield/variable/f513/C513Fields.class"
    )
    paths.forEach {
...
//            browseConstants(f)
  ...
  private fun showLimits(f: JavaClass, threashold: Float = 0.5f) {
    ...
    ) + (0 until f.methodsCount()).map { methodIndex ->
      f.methods()[methodIndex].attributesCount() to "Method #$methodIndex attributes"
    }

    // TODO: why is the compiler returning 'code too large' while we are only close to uint16 and the code size can be uint32?
    val int32 = (0 until f.methodsCount()).flatMap { methodIndex ->
      f.methods()[methodIndex].attributes().mapIndexed { attributeIndex, attributeInfo ->
        when (attributeInfo.info()) {
          is AttrBodyCode -> (attributeInfo.info() as AttrBodyCode).codeLength() to "Method #$methodIndex Code #$attributeIndex"
    ...
    int32.filter { it.first > uint16Threashold }.forEach {
      println(it.second + " count high @ " + it.first)
    }
  }
